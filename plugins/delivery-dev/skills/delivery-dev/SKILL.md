...
An organisation runs many codebases in many languages, and they are deliberately not alike. So
this skill holds two kinds of thing: a few rules that hold everywhere, and a habit for everything
else.
...
The BRD is what you and the customer agreed to - some clients call the same document an
...
argument starts: anything not written in the scope is a CR.
...
- **Commit attribution is per repository.** Some repositories forbid an AI trailer outright. Match `git log`; never add a trailer the repository has not been using.
...
copy rather than retype. Where this skill was installed as a plugin, `/delivery-dev:defect` walks
...
style, not by hand. That one covers the paper; this one covers the code, and they work as a
matched pair.
