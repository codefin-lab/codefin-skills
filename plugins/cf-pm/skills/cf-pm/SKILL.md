...
is in the `cf-ba` skill under `references/sizing.md`. This role owns the budget that
...
here. The defect counting rule lives in `cf-dev`; a steering report quotes it rather than
inventing its own.
