---
description: Walk a defect through the procedure - record it, decide defect or CR, reproduce, trace the blast radius, fix, leave a regression test, run the gates, close with evidence.
argument-hint: [the report, a ticket key, or a path to it]
---

Drive the defect procedure end to end. Read `references/defects.md` in the `cf-dev` skill
...
ahead to the fix because the cause looks obvious - the two things that get defects
...
6. **Fix the cause**, not the symptom. If this decides how something works rather than restoring
   agreed behaviour, write an ADR (`cf-sa`, `templates/adr-template.md`).
