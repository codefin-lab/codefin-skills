...
A defect reopened by the customer costs more than the fix did, and enough of them cost the
relationship. The two causes are that a fix does not cover everywhere it needed to, and that nothing stops the
...
(`cf-sa`, `templates/adr-template.md`), and where the change is architectural, it is the
...
- **If the repository has no such commands, create them from `templates/` as part of this fix.**
  Most repositories do not have them yet. This is how the gates arrive - out of work the
