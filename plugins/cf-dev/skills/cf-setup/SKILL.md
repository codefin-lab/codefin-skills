...
Run the seven steps in `cf-dev`'s `references/detect.md` - instructions for agents, the task
...
| `docs/adr/0001-*.md` | `cf-dev` `templates/adr-template.md` | records that this project adopted these standards, and why |
...
| the test and build targets | `cf-dev` `templates/makefile-test.mk` | `lint`, `build`, `test`, `test-integration`, `test-e2e`, `test-all` |
| the pull request gate | `cf-dev` `templates/workflow-pr.yml` | far easier now than after the repository has grown without one |
...
- **write one real test** against something the code already does, from
  `cf-dev` `templates/unit-test.*`, and watch it pass and then fail when you break the code
  under it; or
