...
- **Anything you publish is a contract.** A schema, an API, a shared library's surface, an event
  ...
  `cf-dev` skill has a fuller procedure and a script for it if you have it installed.
...
(`cf-ba`) - they say what must be true, you say how. The reasoning behind keeping records
in the repository rather than a wiki is in `cf-dev`, `references/knowledge.md`.
